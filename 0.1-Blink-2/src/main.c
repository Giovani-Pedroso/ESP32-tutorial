...
void pisca_task(void *pvParameter){
    ...
    gpio_pad_select_gpio(GPIO_NUM_13);//configura o pino como gpio
    gpio_set_direction(GPIO_NUM_13, GPIO_MODE_OUTPUT);//pino como saida
    printf("okok pisca pisca\n");
    uint8_t cnt=0;

    while(1){
        gpio_set_level(GPIO_NUM_13, cnt%2);//seta o nivel logico
        cnt++;
